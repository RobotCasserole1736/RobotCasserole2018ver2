...
public class JeVoisInterface {
    ...
    private double  tgtX = 0;
    private double  tgtY = 0;
    private double  tgtArea = 0;
    ...
    /**
     * Returns the most recently seen target's X position in the image. 
     * Normailzed to 0-1 range where 0 is full left, 1 is full right.
     */
    public double getTgtXPos() {
        return tgtX/STREAM_WIDTH_PX;
    }

    /**
     * Returns the most recently seen target's Y position in the image. 
     * Normailzed to 0-1 range where 0 is full bottom, 1 is full top.
     */
    public double getTgtYPos() {
        return 1.0 - tgtY/STREAM_HEIGHT_PX;
    }

    /**
     * Returns the most recently seen target's size (area)
     * Normalized such that 1.0 is the full image, 0.5 is half the pixels in the image, etc.
     */
    public double getTgtArea() {
        return tgtArea / (STREAM_HEIGHT_PX * STREAM_WIDTH_PX);
    }
    ...
    public int parsePacket(String pkt, double rx_Time){
        ...
        final int TGT_Y_TOKEN_IDX = 3;
        final int TGT_AREA_TOKEN_IDX = 4;
        ...
        try {
            ...
            tgtX    = Double.parseDouble(tokens[TGT_X_TOKEN_IDX]);
            tgtY    = Double.parseDouble(tokens[TGT_Y_TOKEN_IDX]);
            tgtArea = Double.parseDouble(tokens[TGT_AREA_TOKEN_IDX]);
