...
RIO_MDNS_ADDRESS = 'roboRIO-'+TEAM_NUMBER_STR+'-FRC.local'
RIO_FIXED_IP_ADDRESS = '10.17.36.2'
...
addrs_to_try = [RIO_MDNS_ADDRESS,RIO_FIXED_IP_ADDRESS]
...
while(failed == 1):
    ...
    for addr in addrs_to_try :
        print('Attempting to connect to roboRIO on ftp://' + addr + " ...")
        try:
            ftp = FTP(addr)
            ftp.login()
            failed = 0
